...
class Variable:
    __array__priority__ = 200  # 연산자 우선순위 지정
...
class Pow(Function):
    def __init__(self, c):  # Pow class 초기화 시 지수 c 제공
        self.c = c
...
def add(x0, x1):
    x1 = as_array(x1)  # float, int 등도 np.array로 변환
    return Add()(x0, x1)
...
def rsub(x0, x1):  # 뺄셈에서는 좌우를 구분해야 하므로
    x1 = as_array(x1)
    return Sub()(x1, x0)
...
Variable.__div__ = div  # 왜 truediv로 하면 연산자 오버로딩이 안되는가?
