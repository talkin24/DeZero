...
- 머신러닝, 특히 신경망에서는 뉴턴방법이 좀처럼 사용되지 않음
  - 매개변수가 많아지면 뉴턴 방법의 헤세행렬, 정확히는 헤세행렬의 역행렬 계산에 자원이 너무 많이 소모되는 문제가 있음
    - 매개변수가 n개면 n^2 만큼의 메모리를 사용하며, n\*n의 역행렬 계산에는 n^3만큼 사용함
- 따라서 준 뉴턴방법(QNM)이 사용됨 : 헤세 행렬의 역행렬을 근사해 사용하는 방법의 총칭
  - L-BFGS: 기울기만으로 헤세 행렬을 근사함. 파이토치에 구현됨
  - 다만 딥러닝 분야의 주류는 기울기만을 사용한 최적화(SGD, Momentum, Adam 등)이라서 L-BFGS 등의 준 뉴턴방법을 사용한 예는 많지 않음
- double backprop의 사용예로 Hessian-vector product 계산이 있음
  - 결과만 필요하다면 double backprop을 활용하여 효율적으로 구할 수 있음
  - 벡터와 기술기의 곱, 즉 벡터의 내적을 먼저 구하고, 그 결과로부터 다시 한 번 기울기를 구하면 바로 결과 구할 수 있음.
    - 헤세행렬을 만들지 않아도 되므로 계산 효율이 좋아짐

<br>

## 제4고지: 신경망 만들기

신경망을 잘 다루기 위해선 텐서를 이용해야 함

<br>

### Step37: 텐서를 다루다

- 기울기의 형성과 데이터의 형상은 일치
  - x.shape == x.grad.shape
- 텐서의 미분을 머신러닝에서는 '기울기'라고 함
- 야코비안 매트릭스: 벡터 간의 미분 값의 행렬
- 행렬곱의 계산은 입력 쪽에서 출력 쪽으로 계산하는 forward와 출력에서 입력쪽으로 계산하는 reverse 가 존재
  - forward 모드는 n\*n 행렬을 전파, 그에 비해 reverse 모드는 n개의 백터를 전파함. 즉 reverse모드(역전파)의 계산 효율이 좋음
- 명시적으로 야코비 행렬을 구하여 행렬의 곱을 계산할 필요가 없음. 원소별 연산의 야코비 행렬은 대각 행렬이 되기 때문

### Step38. 형상 변환 함수

- 계산을 원소별로 하지 않는 함수는 텐서의 형상을 고려해야 함
  - 변수의 데이터와 기울기의 형상이 일치하는지 확인 필요
- reshape 함수는 단순히 형상만 변화
- numpy의 reshape는 ndarray 인스턴스의 메서드로 사용할 수 있음. 또한 가변인수로 받을 수도 있음
- `@property` 데코레이터는 인스턴스 변수로 사용할 수 있게 해주는 데코레이터
- np.transpose에 인수를 넣으면 더 범용적으로 사용할 수 있음
...
- 연산에 자주 사용되는 식이 함수화 되어 있으면 메모리를 아낄 수 있음

### Step43. 신경망

- 입력 x와 매개변수 W 사이에서 행렬곱을 선형변환 혹은 아핀변환이라고 함
  - 그러나 신경망 분야에서는 b까지 더하는 것을 선형변환이라고 부름
- 선형 변환은 신경망에서는 완전연결계층에 해당하며, 매개변수 W는 가중치, 매개변수 b는 편향이라고 함
- 덧셈은 흘려보내고 바로 삭제하는 것이 메모리 효율화에 도움됨
- 신경망에서 메모리의 대부분을 차지하는 것이 중간 계산 결과인 텐서임. 특히 큰 텐서를 취급하는 경우 ndarray 인스턴스가 거대해지므로 불필요한 ndarray 인스턴스는 즉시 삭제하는 것이 바람직함
- 약한참조는 메모리 효율을 위해 사용(gc가 약한참조는 메모리 삭제할 수 있음)
- 신경망은 선형 변환의 출력에 비선형 변환을 수행함
- 일반적인 신경망은 '선형변환 -> 활성화함수 -> 선형변환 -> 활성화함수 -> 선형변환 -> ...' 형태로 연속적으로 변환을 수행함
- 신경망에서는 가중치의 초깃값을 무작위로 설정하는 것이 좋음
  - 가중치 초깃값을 0으로 하는 경우, backprop 시 모든 가중치의 값이 똑같이 갱신됨

### Step44. 매개변수를 모아두는 계층

- parameter 클래스는 variable 클래스를 상속받은 것이 다임
  - 단 상속 받았다 하더라도 parameter instance와 variable instance는 구별 가능
- `__setattr__` 은 인스턴스 변수를 설정할 때 호출되는 특수 메서드
  - 이름이 name인 인스턴스 변수에 값으로 value를 전달해줌
- yield는 return처럼 사용할 수 있음
  - 다만 return으 ㄴ처리를 종료하고 값을 반환하는 반면, yield는 처리를 일시중지하고 값을 반환함
  - 따라서 yield를 사용하면 작업을 재개할 수 있음
- 매개변수 관리를 Linear 인스턴스가 맡게 되므로 매갭변수 기울기 재설정과 매개변수 갱신 작업이 전보다 깔끔해짐

### Step45. 계층을 모아두는 계층

- layer 인스턴스 관리를 위해 layer 안에 다른 layer가 들어가는 구조 필요
- 제너레이터를 사용하여 또 다른 제너레이터를 만들고자 할 때는 yield from을 사용함
- 객체지향식 모델 정의 방법은 체이너가 최초로 제안했고, 그 후 파이토치와 텐서플로 등 많은 다른 프레임워크에서 보편적으로 사용하는 방식으로 굳어졌음
- 앞으로 아무리 복잡한 신경망을 구축하더라도 필요한 모든 매개변수를 Model 클래스로 관리 가능
- setattr 함수를 통해 규칙적으로 변화하는 변수 설정 가능
...
- get_item 함수에서 같은 인덱스를 반복 지정하여 동일한 원소를 여러번 빼낼 수 있음
- getitem은 특수 메서드임(`__getitem__`)
- 소프트맥스 함수는 수치를 확률로 변환할 수 있음
- 소프트맥스 함수의 계산은 지수 함수로 이루어지기 때문에 오버플로 문제에 잘 대처해줘야 함
- `clip` 함수는 clip(x, x_min, x_max) 형태로 사용. x가 x_min이하면 x_min으로 변환하고, x_max 이상이면 x_max로 변환

### Step48. 다중 클래스 분류

- minibatch: 데이터를 조금씩 무작위로 모아서 처리하는 데이터 뭉치 단위
- np.random.permutation() 함수를 쓰면 무작위로 섞기 가능(순열)

### Step49. Dataset 클래스와 전처리

- 큰 데이터셋의 경우, 초기화 할때 데이터를 읽지 않고, 데이터에 접근할 때 읽는 것이 효율이 좋음
- 전처리 기능: 특정값 제거, 형상 변형, 이미지 회전 또는 반전, 데이터 수 확장

### Step50. 미니배치를 뽑아주는 DataLoader

- DataLoader는 미니배치 생성과 데이터셋 뒤섞기 등의 기능 제공
- 반복자(iterator): 원소를 반복하여 꺼내줌
- Dataset 인터페이스를 만족하는 인스턴스: `__getitem__`과 `__len__` 메서드를 구현한 클래스로부터 생성된 인스턴스

### Step51. MNIST 학습

- ReLU의 역전파는, 입력 시 0보다 큰 원소의 기울기는 그대로 흘려보내고 0이하 위치의 기울기는 0으로 설정
  - ReLU 함수가 하는 일은 신호를 '통과'시키거나 '봉쇄'하거나 둘중 하나임
  - 순전파 시 통과시킨 원소라면 그대로 통과
  - 순전파 시 막힌 원소라면 통과되지 못함

### 칼럼: 딥러닝 프레임워크

- 최근 딥러닝 프레임워크의 특징

  - Define-by-Run 방식으로 계산 그래프 생성
    - 딥러닝 프레임워크에서 가장 중요한 기능은 '자동미분'
    - Define-by-Run은 시행착오를 많이 겪는 연구/개발 단계에 적합
    - Define-and-Run은 성능이 요구되는 실제 제품이나 IoT 같은 edge환경에서 사용할 때 적합
  - 사전 정의된 다양한 함수와 layer 제공
    - 이 모든 layer는 자동 미분 구조가 지탱
  - 다양한 옵티마이저 제공
  - 모델은 상속을 통해 구현
  - 데이터셋을 관리하는 클래스 제공
  - CPU, GPU 심지어 특정한 ASIC도 활용할 수 있음
  - 성능향상을 위해 Define-and-Run 모드 실행도 지원
...
### Step53. 모델 저장 및 읽어오기
- `self.to_cpu()` -> 데이터가 메인 메모리에 존재함을 보장
- 키보드 인터럽트에 대응하기 위해 try, except 문 사용 가능
...
- 테스트 시에는 모든 뉴런을 사용하지만 그 결과를 약화시켜야 함(다이렉트 드롭아웃)
  - 테스트 비율을 학습시에 살아남은 뉴런의 비율(1 - dropout rate)로 약화

- 역드롭아웃(Inverted Dropout)은 학습 시 미리 뉴런의 값에 1/scale을 곱해두고, 테스트 때 다른처리 없음
  - 테스트 시 별처리가 없어 테스트 속도 향상됨
  - 드롭아웃레이트 동적 변화 가능
  => 많은 프레임워크에서 역드롭아웃 방식 채용

- with문을 사용하면 블록 안에서만 특정 변수가 작동하게 할 수 있음


### Step55. CNN 메커니즘(1)
- 편향은 필터링 후에 더해줌. 하나뿐이라는 점에 주의
- 패딩을 사용하는 주요 이유는 출력 크기 조정
  - 합성곱 연산을 반복 수행하는 깊은 신경망에서 입출력 차원이 다르면 문제가 될 수 있음
- 패딩과 스트라이드는 출력 크기에 영향을 줌


### Step56. CNN 메커니즘(2)
- 미니배치 처리는 차원이 하나 더 추가된 것으로 이해할 수 있음
- 풀링층의 주요 특징
  - 학습하는 매개변수가 없음
  - 채널 수가 변하지 않음
  - 미세한 차이에 robust함
- N(배치), C(채널), H(높이), W(너비) 순서

### Step57. conv2d 함수와 pooling 함수
- numpy에서 for 문을 사용하면 속도가 느려짐
- 텐서곱은 행렬곱의 확장. 축을 지정하여 두 텐서를 곱셈한 후 누적하는 계산
- 합성곱 연산 시 image를 column으로 전재하면 대부분의 경우, 원소 수가 원래보다 많아지므로 메모리를 많이 소비
  - 하지만 행렬곱으로 계산할 시 행렬 라이브러리가 제공하는 최적화된 함수의 혜택을 누릴 수 있음
- assert는 가정 설정문. assert 뒤의 조건이 True가 아니면 AssertError를 발생시킴
- pooling의 적용 영역은 채널마다 독립적으로 전개
  - 따라서 입력 데이터를 풀링하기 쉬운 형태로 전개해주면 그 후 구현은 매우 간단해짐


### Step58. 대표적인 CNN(VGG16)
- 3x3 Conv 64는 커널 크기가 3x3이고 출력 채널 수가 64개라는 뜻
- Pool/2는 2x2 풀링
- Linear 4096은 출력 크기가 4096인 완전연결계층
- VGG16의 특징
  - 3x3 합성곱 층 사용
  - 합성곱층의 채널 수는 풀링하면 2배로 증가(기본적으로, 풀링한다고 반드시 채널 수가 증가하는 건 아님)
  - 완전연결계층에서 드롭아웃 사용
  - ReLU 사용
- 입력 데이터 채널 수는 지정하지 않고, 순전파 시 흐르는 데이터로부터 얻음
  - 이때 가중치 매개변수 또한 초기화됨
- 합성곱 층에서는 4차원 텐서를 처리하지만 fc에서는 2차원 텐서 처리
- yield from을 사용하면 generator 안에 for문을 만들어줄 필요 없음
- 리스트 표현식을 ()소괄호로 묶으면 제너레이터 표현식이 됨
  ```
